...
class DawnStar:#黎明之星形态
    def __init__(self):   
        self.insStockBase = StockBase('2019-01-01','2020-12-31','2020-09','2020-10')
        self.arr_codes = []
        self.arr_names = []
        self.arr_dates = []
        self.arr_profits = []
    ...
    def modelAnalyse(self,stock_code,stock_name):
        ...
        for p in range(1,len(df)-2):
            ...
            n = p + 2
            #第一天收盘要跌
            if df.iloc[p]['Volume'] == 0:
                continue
            if df.iloc[p]['Open'] < df.iloc[p]['Close'] :
                continue
            #第一天的阴线很长
            #if df.iloc[p]['pctChg'] > -3:
            #    continue
            #第一天的实体柱要长
            if (df.iloc[p]['Open']-df.iloc[p]['Close'])/(df.iloc[p]['Close']-df.iloc[p]['Low']) < 2:
                continue
            
            #中间这天要跳空低开
            if df.iloc[m]['High'] > df.iloc[p]['Low']:
                continue                       
            #中间这天的下影线是实体柱的两倍以上
            ...
            if df.iloc[n]['Close'] < df.iloc[p]['High']:
                continue
            if (df.iloc[n]['High'] - df.iloc[n]['Close']) / df.iloc[n]['Close'] > 0.02:
                continue
            #最后这天的量必须大于前两天
            ...
            bFind = True
            bIndex = p + 3
            print("find one bingo total: " + str(len(self.arr_codes)))
            break
        ...
        if bFind:
            ...
            for i in range(bIndex + 1,bIndex+10):
                if df.iloc[i]['High'] / buyPrice > 1.06:
                    profit = 1.06
                    break
            if 0 == profit:
